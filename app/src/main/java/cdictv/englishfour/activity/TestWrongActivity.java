...
public class TestWrongActivity extends AppCompatActivity {
    ...
    private void getInfosFromDB(int i) {
        ...
        if (flag==true){
            mImSc.setBackgroundColor(getResources().getColor(R.color.sc));
        }
    }
    ...
    private void clearInterface() {
        ...
        mImSc.setBackgroundColor(R.drawable.defaults);
    ...
    private void doneWhich(String done) {
        ...
        //查询答案
        Answer answer = DataSupport.find(Answer.class, Long.valueOf(infosList.get(index).getId()));
        String daan;//标准答案
        if (EmptyUtils.isNotEmpty(answer.getDaan1())) {
            daan = answer.getDaan1();
        } else if (EmptyUtils.isNotEmpty(answer.getDaan2())) {
